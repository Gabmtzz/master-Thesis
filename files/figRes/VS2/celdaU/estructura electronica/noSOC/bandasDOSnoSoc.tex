...
\begin{document}
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot3 [only marks, mark size=0.7, blue, raw gnuplot, id=pdos] gnuplot { splot "k_VS2_dos.dat" u 1:($2-0.2016):3 w l;
			};
			\addlegendentry{$\uparrow$}
			\addplot3 [only marks, mark size=0.7, red, raw gnuplot, id=pdosd] gnuplot { splot "k_VS2_dos.dat" u 1:($2-0.2016):4 w points;
			};
		...
		\end{axis}
	    \begin{axis}[legend style={nodes={scale=0.6, transform shape}, font=\large, draw=none}, xshift=7.6cm,	enlargelimits=false,xtick={},xticklabels={}, xlabel=DOS (est./eV),x=0.4cm,ymin=-3, ymax=1.6, yticklabels={}]
	    	
	    	\addplot gnuplot[raw gnuplot, id=dostot1, mark=none, color=blue,solid]{
	    		plot "VS2.dos" u 2:($1-0.201) w l;
	    	}; \addlegendentry{$total$}
    	    \addplot gnuplot[raw gnuplot, id=dostot1, mark=none, color=red,solid]{
    	    	plot "V_d.dat" u 2:($1-0.201) w l;
    	    };
            \addlegendentry{$V_d$}
            \addplot gnuplot[raw gnuplot, id=dostot1, mark=none, color=green,solid]{
            	plot "S_p.dat" u 2:($1-0.201) w l;
            };
            \addlegendentry{$S_p$}
	    	\addplot gnuplot[raw gnuplot, id=dostot1, mark=none, color=blue,solid]{
	    		plot "VS2.dos" u (-1*$3):($1-0.201) w l;
	    	};
	    	\addplot gnuplot[raw gnuplot, id=dostot1, mark=none, color=red,solid]{
	    		plot "V_d.dat" u (-1*$3):($1-0.201) w l;
	    	};
	    	\addplot gnuplot[raw gnuplot, id=dostot1, mark=none, color=green,solid]{
	    		plot "S_p.dat" u (-1*$3):($1-0.201) w l;
	    	};
