...
\begin{document}
	...
	\addplot gnuplot [raw gnuplot, id=test, mark=none,color=blue, solid]{
		plot"ptse2band.txt" using 1:($2-0.13) w l;
	};
    

	\end{axis}
	\begin{axis}[legend style={nodes={scale=0.5, transform shape},  font=\large, draw=none}, xshift=7.6cm,	enlargelimits=false, xmin=0,xtick={},xticklabels={}, xlabel=DOS (est./eV),x=0.4cm,ymin=-3, ymax=2, yticklabels={}]% table
	
	\addplot gnuplot[raw gnuplot, id=test, mark=none, color=blue,solid]{
		plot "ptse2.dos" u 2:($1-0.13) w l;
	};
	\addlegendentry{total}
	\addplot gnuplot[raw gnuplot, id=test, mark=none, color=red,solid]{
		plot "Pt_d_tot.dat" u 2:($1-0.13) w l
	};
	\addlegendentry{$Pt_d $}
	\addplot gnuplot[raw gnuplot, id=test, mark=none, color=green,solid]{
		plot "Se_p_tot.dat" u 2:($1-0.13) w l
	};
	\addlegendentry{$Se_p $}
